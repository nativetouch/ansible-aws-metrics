...
if ($name) {
  chomp($machine_name= `ec2-describe-tags --filter \"resource-type=instance\" --filter \"resource-id=\$(ec2-metadata -i | cut -d ' ' -f2)\" --filter \"key=Name\" | cut -f5`);
...
sub add_metric
{
  ...
  if ($name) {
     %dims = (('MachineName' => $machine_name), %xdims);
     add_single_metric($name, $unit, $value, \%dims);
  }
